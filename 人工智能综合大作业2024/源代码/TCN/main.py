...
num_channels = [16, 32, 64]  # TCN层的通道数
kernel_size = 3  # 卷积核大小
dropout = 0.2  # dropout率

# 实例化TCN模型
net = model(input_size, output_size, num_channels, kernel_size=2, dropout=0.2)
...
for i in range(0, 7):
    ...
    with torch.no_grad():  # 禁用梯度计算，以节省内存
        ...
        np.save(f"./TCN/Feature_{i}_y_pred.npy", pred)  # 保存预测结果到文件
