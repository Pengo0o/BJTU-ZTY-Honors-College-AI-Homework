...
class TemporalBlock(nn.Module):
    ...
    def init_weights(self):
        self.conv1.weight.data.normal_(0, 0.01)  # 初始化第一个卷积层的权重
        self.conv2.weight.data.normal_(0, 0.01)  # 初始化第二个卷积层的权重
        if self.downsample is not None:
            self.downsample.weight.data.normal_(0, 0.01)  # 初始化下采样层的权重
