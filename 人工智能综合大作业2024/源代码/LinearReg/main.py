...
for i in range(0, 7):
    ...
    lr.fit(X_train.reshape(X_train.shape[0], -1), y_train_feature)  # 训练模型
    y_pred = lr.predict(X_test.reshape(X_test.shape[0], -1))  # 预测测试集
    ...
    # 保存预测结果和真实标签到文件
    np.save(f"./LinearReg/Feature_{i}_y_pred.npy", y_pred)
    np.save(f"./truth_{i}.npy", y_test_feature)
